  ...
  return (
    <div style={{ padding: '60px 20px', maxWidth: '1100px', margin: '0 auto' }}>
      ...
      <header style={{ marginBottom: '50px' }}>
        <h1 style={{ color: '#FFFFFF', fontSize: '2.8rem' }}>Art & Culture</h1>
      </header>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '40px' }}>
        {places.map((place) => (
          <Link to={`/ArtCultureDetail/${place.id}`} key={place.id} style={{ textDecoration: 'none', backgroundColor: '#1E1E1E', borderRadius: '12px', overflow: 'hidden' }}>
            <img src={place.primaryImagePath ? `${MEDIA_BASE_URL}${place.primaryImagePath}` : 'https://images.unsplash.com/photo-1518998053901-5348d3961a04?w=800'} alt={place.officialName} style={{ width: '100%', height: '220px', objectFit: 'cover' }} />
            <div style={{ padding: '20px' }}>
              <h3 style={{ color: '#FFFFFF' }}>{place.officialName}</h3>
              <div style={{ color: '#4DA8DA', marginTop: '10px' }}>View Details &rarr;</div>
            </div>
          </Link>
        ))}
      </div>
